...
> **Last Updated:** February 2026 - Major enhancements including parallel extraction architecture, 8-stage objection validation with category re-mapping + deduplication, 3-call summary extractor pipeline (summary + PA Stage 1 intent classification + PA Stage 2 revenue impact check) with noise filtering, hybrid diarization with timestamp alignment, and tenant configuration integration.
...
| **Multi-Stage Objection Detection** | 8-stage pipeline with programmatic validation, category re-mapping, deduplication to prevent hallucinations |
...
│  │  │  │ (3-call       │ │               │ │               │ │         ││ │   │
│  │  │  │  pipeline)    │ │ • SOP eval    │ │ • 8-STAGE     │ │• 4 LLM  ││ │   │
│  │  │  │               │ │ • Stage check │ │   PIPELINE    │ │  calls  ││ │   │
│  │  │  │ Call 1: Summ  │ │ • Specificity │ │   + DEDUP     │ │• BANT   ││ │   │
│  │  │  │ Call 2: PA S1 │ │ • Call-type   │ │   (prevents   │ │• Appt   ││ │   │
│  │  │  │ Call 3: PA S2 │ │   aware       │ │   hallucin.)  │ │• Address││ │   │
│  │  │  └───────────────┘ └───────────────┘ └───────────────┘ └─────────┘│ │   │
│  │  │                                                                     │ │   │
│  │  │  Objection 8-Stage Pipeline + Dedup:                               │ │   │
│  │  │  1. Extract candidates  2. Classify  3. Category re-map            │ │   │
│  │  │  4. Validate transcript  5. Confidence  6. Business filters        │ │   │
│  │  │  7. Speaker validation  8. Cross-validation → Deduplication        │ │   │
│  │  │                                                                     │ │   │
...
#### Objection Extraction (8-Stage Pipeline + Deduplication)

The objection extractor uses an 8-stage pipeline with post-pipeline deduplication:

| Stage | Name | Type | Description |
|-------|------|------|-------------|
| 1 | Extract Candidates | LLM | Extract raw customer statements |
| 2 | Classify | LLM | Classify as objection/non-objection with category |
| 3 | Category Re-map | LLM | Validate category against enum, assign sub_objection for "Other" (ID 9) |
| 4 | Validate Transcript | Programmatic | Verify objection text exists in transcript |
| 5 | Confidence Scores | Programmatic | Calculate evidence-based confidence scores |
| 6 | Business Filters | Programmatic | Apply business logic, normalize categories, safety net for sub_objection |
| 7 | Speaker Validation | LLM | Verify speaker is customer (not rep) |
| 8 | Cross-Validation | LLM | Final quality check — catches false positives (past company references, cooperation, questions) and hints at semantic duplicates |
| Post | Deduplication | Programmatic | SequenceMatcher + Jaccard word similarity (threshold 0.5) removes semantic duplicates |

**Cross-chunk dedup**: `_merge_with_previous` uses fuzzy matching (SequenceMatcher + Jaccard, threshold 0.5) instead of exact text match.

**Sub-objection field**: When `category_id=9` ("Other"), `sub_objection` contains the descriptive label (e.g., "Trust/Quality Concern"). All 9 specific categories (IDs 1-8, 10) have `sub_objection=null`.
...
│   │
│   └── extractors/
│       ├── summary_extractor.py      # 3-call pipeline: summary + PA Stage 1 (intent) + PA Stage 2 (revenue impact)
│       ├── compliance_extractor.py   # SOP compliance (call-type aware)
│       ├── objection_extractor.py    # 8-stage anti-hallucination pipeline + dedup
